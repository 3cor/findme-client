...
class EPScreen():
    ...
    def update_screen(self):
        ...
        if self.image_old is None or self.need_to_refresh():
            if self.screen_orientation == 'landscape':
                self.device.display_frame(self.device.get_frame_buffer(
                    self.image_live.rotate(90, expand=1)))
            else:
                self.device.display_frame(
                    self.device.get_frame_buffer(self.image_live))
    ...
    def add_text_middle(self, y, text, font, fill, width=-1, offset_x=0):
        ...
        self.draw.text((offset_x + (width) / 2 - w / 2, y -
                        offset_y), text, font=font, fill=fill)
